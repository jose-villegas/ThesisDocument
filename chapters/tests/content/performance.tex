...
Considerando como tiempos interactivos todo resultado por debajo de $33.3 ms$ o aproximadamente 30 cuadros por segundo. Nuestra implementación logra colocarse por debajo de estos tiempos en todos los casos excepto los casos que comprenden resoluciones de volumenes mayores a $256^3$.

\subsubsection{Densidad Geometrica y Velocidad de Voxelizacion.}
La cantidad de triangulos dentro del volumen que representa un voxel afecta la velocidad de voxelizacion, esto debido a que este proceso requiere sincronizacion entre distintos hilos por fragmento. A mayor cantidad de triangulos por voxel mayor es la cantidad de fragmentos generados, cada uno de estos fragmentos compite por escribir en la misma posicion del volumen.

Se puede observar este efecto especialmente en la escena Conference. Esta es una escena pequena en escala, sin embargo de todas las escenas completas es la que posee mayor cantidad de triangulos. Tambien es notable como disminuye el tiempo de voxelizacion para algunas escenas al aumentar la resolucion del volumen y como aumenta al utilizar menores resoluciones, especialmente el salto entre $128^3$ a $6^34$.

\subsubsection{Vacuidad y Velocidad de Trazado.}
