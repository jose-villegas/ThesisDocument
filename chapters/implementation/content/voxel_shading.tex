...
\noindent Para disminuir este problema se calcula la atenuación normal por cada cara del vóxel según la dirección de la luz $\Psi$ y luego se pondera cada atenuación normal por cara frontal segun el vector normal con el peso de cada eje en el vector normal promedio del vóxel, este procedimiento se puede observar en el Código \ref{Shading2}. En la Figura \ref{fig:compositve_vshading} se puede observar la composición para el sombreado final y la diferencia con Lambert tradicional. Además del método Lambert tradicional a este método se le denomino Lambert direccional ponderado.
\\
